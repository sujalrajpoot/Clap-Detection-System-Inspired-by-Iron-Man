...
from audio_inference import AudioModelHandler
    ...
    def record_and_detect(self) -> None:
        ...
        with self.stream:
            ...
            while True:
                ...
                temp_filename = os.path.join(self.directory, "temp.wav")
...
def detect_claps(device_index: int = 1, chunk_duration: float = 0.5) -> None:
    """
    Detects claps using audio input from the specified device.

    Args:
        device_index (int): Index of the input device to use. Default is 1.
    """
    
    model_path = r"clap-detection-model.pth"
